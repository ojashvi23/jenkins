...
function Editaddress(props) {
    ...
    let onOnclickHandler = (e) => {
        ...
        console.log("type  api edit data", type_value);
        let name_value = textinput2.current.value;
        console.log(" name api edit data ", name_value);
        // let location_value = textinput1.current.value;
        // console.log("location api edit  data ", location_value);
        const location_value = localStorage.getItem('address');

        let address_value = textinput3.current.value;
        console.log("address api edit data", address_value);
        axios.post(`https://market.jprtechnosoft.com/api/editcustomeraddress?customer_id=${Customer}&type=${type_value}&full_name=${name_value}&location=${location_value}&address=${address_value}`)
            .then(res => {
                console.log("response api edit", res);
    ...
    return (
        <>
            <p style={{ color: 'green', marginLeft: '190px', marginTop: '45px' }} onClick={() => setModalShow(true)}>Edit</p>
